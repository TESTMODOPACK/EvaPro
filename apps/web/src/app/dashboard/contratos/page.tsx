...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  pending_signature: { label: 'Pendiente de Firma', color: '#f59e0b' },
  active: { label: 'Vigente', color: '#10b981' },
  expired: { label: 'Expirado', color: '#ef4444' },
  superseded: { label: 'Reemplazado', color: '#6366f1' },
};
  ...
  if (error) return (
    <div style={{ padding: '2rem 2.5rem' }}>
      <div className="card" style={{ padding: '2rem', textAlign: 'center', borderLeft: '4px solid var(--danger)' }}>
        <p style={{ color: 'var(--danger)', fontWeight: 600 }}>Error al cargar contratos</p>
  ...
  return (
    <div style={{ padding: '2rem 2.5rem', maxWidth: '1100px' }}>
      <div className="animate-fade-up" style={{ marginBottom: '1rem' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 800, marginBottom: '0.25rem' }}>Contratos y Documentación Legal</h1>
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>
          Documentos contractuales entre su organización y Ascenda Performance
        </p>
      </div>
      ...
      {showGuide && (
        <div className="card animate-fade-up" style={{ borderLeft: '4px solid var(--accent)', padding: '1.5rem', marginBottom: '1.5rem' }}>
          <h3 style={{ fontSize: '1rem', fontWeight: 800, marginBottom: '1rem', color: 'var(--accent)' }}>Guía: Contratos y Documentación Legal</h3>
      ...
      {contracts.length === 0 ? (
        <div className="card" style={{ padding: '3rem', textAlign: 'center' }}>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>No hay contratos registrados para su organización.</p>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', marginTop: '0.5rem' }}>Los contratos serán creados por el equipo de Ascenda cuando se formalice la suscripción.</p>
        </div>
      ) : (
              ...
              <div key={c.id} className="card animate-fade-up" style={{ overflow: 'hidden', padding: 0 }}>
                ...
                <button type="button" onClick={() => setExpandedId(isExpanded ? null : c.id)}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flex: 1 }}>
                    <div style={{ width: 36, height: 36, borderRadius: '8px', background: `${status.color}15`, color: status.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem', flexShrink: 0 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    {sigs.length > 0 && <span style={{ fontSize: '0.72rem', color: 'var(--success)', fontWeight: 600 }}>Firmado</span>}
                    <span style={{ padding: '0.15rem 0.5rem', borderRadius: '999px', fontSize: '0.7rem', fontWeight: 600, background: `${status.color}15`, color: status.color }}>
                      {status.label}
                    </span>
                ...
                {isExpanded && (
                  <div style={{ padding: '1.25rem', borderTop: '1px solid var(--border)' }}>
                    ...
                    {sigs.length > 0 && (
                      <div style={{ marginBottom: '1rem' }}>
                        <div style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>Firmas</div>
                    ...
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.75rem', display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                      {c.effectiveDate && <span>Vigencia: {new Date(c.effectiveDate).toLocaleDateString('es-CL')}</span>}
                      {c.expirationDate && <span>Vence: {new Date(c.expirationDate).toLocaleDateString('es-CL')}</span>}
